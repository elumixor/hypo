...
export class Projectile {
  constructor(readonly mesh: THREE.Mesh, readonly vel: THREE.Vector3, readonly fromPlayer: boolean) {}
}
...
export class Projectiles {
  ...
  update(dt: number, scene: THREE.Scene, player: Player, enemies: Enemy[], onPlayerHit: ()=>void, onEnemyKilled: (e: Enemy)=>void) {
    for (let i=this.list.length-1;i>=0;i--) {
      const p = this.list[i]; if (!p) continue;
      p.mesh.position.addScaledVector(p.vel, dt);
      if (p.mesh.position.length()>30) { this.disposeAt(i, scene); continue; }
      if (p.fromPlayer) {
        for (const e of enemies) {
          if (p.mesh.position.distanceTo(e.mesh.position)<0.6) {
            e.hp -= 1; this.disposeAt(i, scene);
            if (e.hp<=0) onEnemyKilled(e);
            break;
          }
        }
      } else if (p.mesh.position.distanceTo(player.mesh.position)<0.6) {
        onPlayerHit(); this.disposeAt(i, scene);
      }
    }
  }
