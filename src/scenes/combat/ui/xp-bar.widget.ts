...
export class XPBarWidget extends Widget {
  ...
  override async init() {
    ...
    this.xpBarBg.rect(0, 0, this.barWidth, this.barHeight)
      .fill({ color: 0x333333 })
      .stroke({ color: 0x666666, width: 1 });

    // Create XP bar fill
    this.xpBarFill.rect(0, 0, 0, this.barHeight)
      .fill({ color: 0x4a90e2 });
  ...
  private readonly updateXPBar = () => {
    ...
    this.xpBarFill.rect(0, 0, fillWidth, this.barHeight)
      .fill({ color: 0x4a90e2 });
  };
  ...
  private resize({ width, height }: ResizeData) {
    ...
    this.xpBarContainer.position.set(
      -width / 2 + margin,
      height / 2 - margin - this.barHeight
    );
